...
def register_tools(mcp):
    ...
    def reload_webapp(domain: str) -> str:
        ...
        This reloads uwsgi-based web applications on PythonAnywhere. For ASGI-based web applications, use the `reload_website` tool instead. Any changes to the code require a reload to take effect.

        Args:
            domain (str): The domain name of the web application to reload (e.g., 'alice.pythonanywhere.com').
    ...
    def reload_website(domain: str) -> str:
        ...
        This is for ASGI-based web applications on PythonAnywhere. For uWSGI-based web applications, use the `reload_webapp` tool instead. Any changes to the code require a reload to take effect.

        Args:
            domain (str): The domain name of the website to reload (e.g., 'alice.pythonanywhere.com').
    ...
    def list_websites() -> List[dict[str, Any]]:
        """
        Return info dictionaries for every ASGI website configured for the current user.
    ...
    def create_website(domain_name: str, command: str) -> dict:
        ...
        This function is primarily used to create a new ASGI-based website on PythonAnywhere, but it can also be used for
        any website that communicates via a Unix domain socket. The provided command must use the `DOMAIN_SOCKET`
        environment variable to bind to the socket.

        Args:
            domain_name (str): The domain name for the new website (e.g., `yourusername.pythonanywhere.com` or `www.mydomain.com`).
            command (str): The command to run for the new website. Examples:
                - For FastAPI (ASGI):
                    `/usr/local/bin/uvicorn myapp:app --app-dir '/home/yourusername/fastapi/' --uds \${DOMAIN_SOCKET}`
                - For async Django:
